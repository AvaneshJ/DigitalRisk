...
Client-Side UUIDs: The frontend generates a unique transaction_id (UUIDv4) for every distinct action.

Database Primary Keys: The transactions table in PostgreSQL uses this id as its Primary Key.

Execution: If two identical requests hit the server, the database's unique constraint acts as the final source of truth. The backend explicitly catches the resulting IntegrityError and returns a graceful duplicate warning, completely preventing double-charging.
